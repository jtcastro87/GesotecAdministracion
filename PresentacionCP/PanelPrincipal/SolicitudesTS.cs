...
{
    public partial class SolicitudesTS : SolicitudesBase
    {
        public SolicitudesTS()
        {
            InitializeComponent();
        }
        ...
        private void SolicitudesTS_Load(object sender, EventArgs e)
        {
            FillGridRequests();
            DisableColumnGrid();
            FillComboUsuarios();
        ...
        public override void FillGridRequests()
        {
            try
            {
                ...
                this.gridSolicitudes.DataSource = requestsDA.FillGridRequestsPerUser(UserUp.UserID);
                DisableColumnGrid();
            }
            catch (Exception ex)
            {
        ...
        {
            if (radioSeleccionado.Equals("Estatus"))
            {
                if ((this.cbFiltrado.GetItemText(this.cbFiltrado.SelectedItem).ToString()) != "- Seleccionar -")
                {
                    try
                    {
                        ...
                        this.gridSolicitudes.DataSource = requestsDA.GetRequestsForStatusPerUser(status,UserUp.UserID);
                        DisableColumnGrid();
